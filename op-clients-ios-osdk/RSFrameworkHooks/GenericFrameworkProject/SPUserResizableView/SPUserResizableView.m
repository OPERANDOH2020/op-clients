...
@property (assign, nonatomic) BOOL touchesDidMove;
@property (assign, nonatomic) BOOL touchesEndedOrCancelled;
@property (assign, nonatomic) NSInteger numOfTouchesBegan;
@property (strong, nonatomic) NSDate *dateOfFirstTouchesBegan;
@end
...
- (SPUserResizableViewAnchorPoint)anchorPointForTouchLocation:(CGPoint)touchPoint {
    ...
    if (self.disableResizing) {
        return centerPoint.anchorPoint;
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (!self.keepEditingHandlesHidden) {
        [borderView setHidden:NO];
    }
    
    self.numOfTouchesBegan++;
    if (self.numOfTouchesBegan == 1) {
        self.dateOfFirstTouchesBegan = [NSDate date];
    } else {
        NSTimeInterval secondsAfterLastTap = [[NSDate date] timeIntervalSinceDate:self.dateOfFirstTouchesBegan];
        self.numOfTouchesBegan = 0;
        
//        if (secondsAfterLastTap < 1 && !self.touchesDidMove) {
//            if ([self.delegate respondsToSelector:@selector(userResizableViewDidReceiveDoubleTap:)]) {
//                [self.delegate userResizableViewDidReceiveDoubleTap:self];
//            }
//        }
    }
    ...
    //
    self.touchesDidMove = false;
    self.touchesEndedOrCancelled = false;
...
//        if (self.touchesEndedOrCancelled && !self.touchesDidMove) {
//            if ([self.delegate respondsToSelector:@selector(userResizableViewDidReceiveTap:)]) {
//                [self.delegate userResizableViewDidReceiveTap:self];
//            }
//        }
//    });
//    
...
//        if (!self.touchesEndedOrCancelled && !self.touchesDidMove) {
//            if ([self.delegate respondsToSelector:@selector(userResizableViewDidReceiveLongPress:)])  {
//                [self.delegate userResizableViewDidReceiveLongPress:self];
//            }
//        }
//    });
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    self.touchesEndedOrCancelled= true;
    self.touchesDidMove = false;
...
- (void)translateUsingTouchLocation:(CGPoint)touchPoint {
    ...
    if ([self.delegate respondsToSelector:@selector(userResizableViewDidChangeFrame:)]) {
        [self.delegate userResizableViewDidChangeFrame:self];
    }
